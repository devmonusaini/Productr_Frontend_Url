...
const AddProducts = ({ setAddProductOpen ,fetchProducts }) => {
...
const handleSubmit = async (e) => {
  ...
  if (!formData.productName || !formData.mrp || !formData.price || !formData.quantity) {
  ...
  return (
   <div className="max-w-[400px] w-full  max-h-[95vh] overflow-y-auto bg-white rounded-xl shadow-lg">
          ...
          {formData.productImage.length > 0 ? (
            <div className="grid grid-cols-5 gap-2 mt-3">
              {formData.productImage.map((img, index) => (
                <div key={index} className="relative p-3  border border-[#D4D4D4] rounded">
                  <img
                    src={img.preview}
                    alt=""
      ...
      <div className="w-full flex justify-end items-center navbarBackground p-2">
        <button
          type="submit"
          className="bg-[#071074] text-white p-3 hover:bg-[#050c5a] transition text-sm rounded-xl cursor-pointer"
        >
          Create
        </button>
      </div>
